...
public interface PairRepository extends JpaRepository<Pair, Long> {
    ...
    @Query(nativeQuery = true, value = "SELECT * FROM pairs_prices WHERE price = (SELECT MIN(price) FROM pairs_prices WHERE crypto_currency=:currency_name) LIMIT 1;")
    Optional<Pair> findMinPrice(@Param(value = "currency_name") String cryptoCurrency);

    @Query(nativeQuery = true, value = "SELECT * FROM pairs_prices WHERE price = (SELECT MAX(price) FROM pairs_prices WHERE crypto_currency=:currency_name) LIMIT 1;")
    Optional<Pair> findMaxPrice(@Param(value = "currency_name") String cryptoCurrency);

    @Transactional
    @Modifying
    @Query(nativeQuery = true, value = "INSERT INTO pairs_prices(crypto_currency, currency, price, date) VALUES(:symbol1, :symbol2, :price, :date)")
